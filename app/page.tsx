...
const experiments = [
  { number: "01", title: "Research field notes", description: "把复杂的问题拆成可以理解、可以分享、也可以继续追问的小块。", tag: "in progress" },
  { number: "02", title: "Pop culture archive", description: "记录 Taylor Swift 与 TWICE 带来的情绪、灵感和那些突然被点亮的时刻。", tag: "collecting" },
  { number: "03", title: "Tiny life systems", description: "为学习、生活和下一次出发做一些轻盈的小型实验。", tag: "testing" },
];
  ...
  return (
    <main>
      <nav className="topbar" aria-label="Main navigation">
        ...
        <a className="nav-status" href="#contact"><span className="status-dot" aria-hidden="true" /> available for a hello</a>
      </nav>
      ...
      <section className="hero section-shell" id="top">
        <div className="hero-copy">
          ...
          <p className="hero-intro">研二在读。喜欢把问题想深一点，也喜欢把生活过得闪亮一点。</p>
          <div className="hero-actions">
            <a className="button button-primary" href="#experiments">enter the lab <span aria-hidden="true">↘</span></a>
      ...
      <section className="about section-shell" id="about">
        ...
        <div className="about-grid">
          <h2>Somewhere between<span>the question</span>and the next song.</h2>
          <div className="about-copy">
            <p>我是 Alison，一名正在读研二的学生。这里不是一份冷冰冰的履历，而是一个会持续更新的小空间：放研究，放灵感，放那些让我想继续往前走的东西。</p>
            <p className="muted-copy">If you found your way here, stay for a while. There is probably a thought, a tab, or a song waiting for you.</p>
        ...
        <div className="experiment-list">
          {experiments.map((experiment) => (
            <article className="experiment-card" key={experiment.number}>
              <div className="experiment-topline"><span>{experiment.number}</span><span className="experiment-tag">{experiment.tag}</span></div>
      ...
      <section className="signals section-shell" id="signals">
        ...
        <div className="signals-grid">
          {signals.map(([number, label, value]) => (
            <div className="signal-row" key={number}><span className="signal-number">{number}</span><span className="signal-label">{label}</span><strong>{value}</strong></div>
          ))}
        </div>
        <div className="playlist-card">
          <div><p className="eyebrow"><span>now playing</span></p><h2>main character energy</h2></div>
          <div className="sound-bars" aria-hidden="true"><i /><i /><i /><i /><i /><i /><i /><i /></div>
      ...
      <section className="music-gateway section-shell" id="music">
        <div className="section-heading">
          <p className="eyebrow"><span>05</span> music archive / choose a universe</p>
      ...
      <section className="contact section-shell" id="contact">
        <div className="contact-inner">
          <p className="eyebrow"><span>06</span> end of current session</p>
      ...
      <footer className="footer section-shell"><span>ALISON / PERSONAL LAB</span><span>made with curiosity + good music</span><span>© 2026</span></footer>
    </main>
  );
}
